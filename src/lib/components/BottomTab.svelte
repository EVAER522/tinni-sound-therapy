<script>
  ...
  function getIcon(id, active) {
    const a = active ? 'currentColor' : 'var(--tab-inactive-color, rgba(255,255,255,0.45))';
    switch (id) {
...
<style>
  :root {
    --tab-inactive-color: rgba(255,255,255,0.45);
    --tab-active-color: #fff;
    --tab-hover-color: rgba(255,255,255,0.75);
  }
  :global(.light-mode) {
    --tab-inactive-color: rgba(0,0,0,0.38);
    --tab-active-color: #1d1d1f;
  ...
  .tab-item { flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; gap: 2px; padding: 6px 0; background: none; border: none; cursor: pointer; color: var(--tab-inactive-color); transition: color 0.22s var(--spring-smooth), transform 0.18s var(--spring-bounce); position: relative; -webkit-tap-highlight-color: transparent; }
  ...
  .tab-item:hover { color: var(--tab-hover-color); transform: translateY(-1px); }
